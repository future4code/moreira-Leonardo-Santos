// Crie um pacote do Node  ok
// contendo um script chamado search-country-by-name.   ok
//  Ele deve receber uma string pelo terminal    ok
//e imprimir, no console, uma tabela com os países cujo nome contém a string  informada   ok 
const countries = [
    {
       id: 0,
       name: "África do Sul",
       capital: "Pretória",
       continent: "África"
    },
    {
       id: 1,
       name: "Angola",
       capital: "Luanda",
       continent: "África"
    },
    {
       id: 2,
       name: "Argélia",
       capital: "Argel",
       continent: "África"
    },
    {
       id: 3,
       name: "Benim",
       capital: "Porto Novo",
       continent: "África"
    },
    {
       id: 4,
       name: "Botsuana",
       capital: "Gaborone",
       continent: "África"
    },
    {
       id: 5,
       name: "Burquina Faso",
       capital: "Uagadugu",
       continent: "África"
    },
    {
       id: 6,
       name: "Burúndi",
       capital: "Bujumbura",
       continent: "África"
    },
    {
       id: 7,
       name: "Cabo Verde",
       capital: "Praia",
       continent: "África"
    },
    {
       id: 8,
       name: "Camarões",
       capital: "Iaundé",
       continent: "África"
    },
    {
       id: 9,
       name: "Chade",
       capital: "Jamena",
       continent: "África"
    },
    {
       id: 10,
       name: "Comores",
       capital: "Moroni",
       continent: "África"
    },
    {
       id: 11,
       name: "Congo-Brazzaville",
       capital: "Brazavile",
       continent: "África"
    },
    {
       id: 12,
       name: "Costa do Marfim",
       capital: "Iamussucro",
       continent: "África"
    },
    {
       id: 13,
       name: "Egito",
       capital: "Cairo",
       continent: "África"
    },
    {
       id: 14,
       name: "Eritreia",
       capital: "Asmara",
       continent: "África"
    },
    {
       id: 15,
       name: "Etiópia",
       capital: "Adis Abeba",
       continent: "África"
    },
    {
       id: 16,
       name: "Gabão",
       capital: "Libreville",
       continent: "África"
    },
    {
       id: 17,
       name: "Gâmbia",
       capital: "Banjul",
       continent: "África"
    },
    {
       id: 18,
       name: "Gana",
       capital: "Acra",
       continent: "África"
    },
    {
       id: 19,
       name: "Guiné",
       capital: "Conacri",
       continent: "África"
    },
    {
       id: 20,
       name: "Guiné Equatorial",
       capital: "Malabo",
       continent: "África"
    },
    {
       id: 21,
       name: "Guiné-Bissau",
       capital: "Bissau",
       continent: "África"
    },
    {
       id: 22,
       name: "Jibuti",
       capital: "Jibuti",
       continent: "África"
    },
    {
       id: 23,
       name: "Lesoto",
       capital: "Maseru",
       continent: "África"
    },
    {
       id: 24,
       name: "Libéria",
       capital: "Monróvia",
       continent: "África"
    },
    {
       id: 25,
       name: "Líbia",
       capital: "Trípoli",
       continent: "África"
    },
    {
       id: 26,
       name: "Madagáscar",
       capital: "Antananarivo",
       continent: "África"
    },
    {
       id: 27,
       name: "Maláui",
       capital: "Lilôngue",
       continent: "África"
    },
    {
       id: 28,
       name: "Mali",
       capital: "Bamaco",
       continent: "África"
    },
    {
       id: 29,
       name: "Marrocos",
       capital: "Rebate",
       continent: "África"
    },
    {
       id: 30,
       name: "Maurícia",
       capital: "Porto Luís",
       continent: "África"
    },
    {
       id: 31,
       name: "Mauritânia",
       capital: "Nuaquechote",
       continent: "África"
    },
    {
       id: 32,
       name: "Moçambique",
       capital: "Maputo",
       continent: "África"
    },
    {
       id: 33,
       name: "Namíbia",
       capital: "Vinduque",
       continent: "África"
    },
    {
       id: 34,
       name: "Níger",
       capital: "Niamei",
       continent: "África"
    },
    {
       id: 35,
       name: "Nigéria",
       capital: "Abuja",
       continent: "África"
    },
    {
       id: 36,
       name: "Quénia",
       capital: "Nairóbi",
       continent: "África"
    },
    {
       id: 37,
       name: "República Centro-Africana",
       capital: "Bangui",
       continent: "África"
    },
    {
       id: 38,
       name: "República Democrática do Congo",
       capital: "Quinxassa",
       continent: "África"
    },
    {
       id: 39,
       name: "Ruanda",
       capital: "Quigali",
       continent: "África"
    },
    {
       id: 40,
       name: "São Tomé e Príncipe",
       capital: "São Tomé",
       continent: "África"
    },
    {
       id: 41,
       name: "Seicheles",
       capital: "Vitória",
       continent: "África"
    },
    {
       id: 42,
       name: "Senegal",
       capital: "Dacar",
       continent: "África"
    },
    {
       id: 43,
       name: "Serra Leoa",
       capital: "Freetown",
       continent: "África"
    },
    {
       id: 44,
       name: "Somália",
       capital: "Mogadíscio",
       continent: "África"
    },
    {
       id: 45,
       name: "Suazilândia",
       capital: "Lobamba",
       continent: "África"
    },
    {
       id: 46,
       name: "Sudão",
       capital: "Cartum",
       continent: "África"
    },
    {
       id: 47,
       name: "Sudão do Sul",
       capital: "Juba",
       continent: "África"
    },
    {
       id: 48,
       name: "Tanzânia",
       capital: "Dodoma",
       continent: "África"
    },
    {
       id: 49,
       name: "Togo",
       capital: "Lomé",
       continent: "África"
    },
    {
       id: 50,
       name: "Tunísia",
       capital: "Tunes",
       continent: "África"
    },
    {
       id: 51,
       name: "Uganda",
       capital: "Campala",
       continent: "África"
    },
    {
       id: 52,
       name: "Zâmbia",
       capital: "Lusaca",
       continent: "África"
    },
    {
       id: 53,
       name: "Zimbábue",
       capital: "Harare",
       continent: "África"
    },
    {
       id: 54,
       name: "Antiga e Barbuda",
       capital: "São João",
       continent: "América"
    },
    {
       id: 55,
       name: "Argentina",
       capital: "Buenos Aires",
       continent: "América"
    },
    {
       id: 56,
       name: "Bahamas",
       capital: "Nassau",
       continent: "América"
    },
    {
       id: 57,
       name: "Barbados",
       capital: "Bridgetown",
       continent: "América"
    },
    {
       id: 58,
       name: "Belize",
       capital: "Belmopã",
       continent: "América"
    },
    {
       id: 59,
       name: "Bolívia",
       capital: "Sucre",
       continent: "América"
    },
    {
       id: 60,
       name: "Brasil",
       capital: "Brasília",
       continent: "América"
    },
    {
       id: 61,
       name: "Canadá",
       capital: "Otava",
       continent: "América"
    },
    {
       id: 62,
       name: "Chile",
       capital: "Santiago",
       continent: "América"
    },
    {
       id: 63,
       name: "Colômbia",
       capital: "Bogotá",
       continent: "América"
    },
    {
       id: 64,
       name: "Costa Rica",
       capital: "São José",
       continent: "América"
    },
    {
       id: 65,
       name: "Cuba",
       capital: "Havana",
       continent: "América"
    },
    {
       id: 66,
       name: "Dominica",
       capital: "Roseau",
       continent: "América"
    },
    {
       id: 67,
       name: "Equador",
       capital: "Quito",
       continent: "América"
    },
    {
       id: 68,
       name: "Estados Unidos",
       capital: "Washington, D.C.",
       continent: "América"
    },
    {
       id: 69,
       name: "Granada",
       capital: "São Jorge",
       continent: "América"
    },
    {
       id: 70,
       name: "Guatemala",
       capital: "Cidade da Guatemala",
       continent: "América"
    },
    {
       id: 71,
       name: "Guiana",
       capital: "Georgetown",
       continent: "América"
    },
    {
       id: 72,
       name: "Haiti",
       capital: "Porto Príncipe",
       continent: "América"
    },
    {
       id: 73,
       name: "Honduras",
       capital: "Tegucigalpa",
       continent: "América"
    },
    {
       id: 74,
       name: "Jamaica",
       capital: "Kingston",
       continent: "América"
    },
    {
       id: 75,
       name: "México",
       capital: "Cidade do México",
       continent: "América"
    },
    {
       id: 76,
       name: "Nicarágua",
       capital: "Manágua",
       continent: "América"
    },
    {
       id: 77,
       name: "Panamá",
       capital: "Cidade do Panamá",
       continent: "América"
    },
    {
       id: 78,
       name: "Paraguai",
       capital: "Assunção",
       continent: "América"
    },
    {
       id: 79,
       name: "Peru",
       capital: "Lima",
       continent: "América"
    },
    {
       id: 80,
       name: "República Dominicana",
       capital: "São Domingos",
       continent: "América"
    },
    {
       id: 81,
       name: "Salvador",
       capital: "São Salvador",
       continent: "América"
    },
    {
       id: 82,
       name: "Santa Lúcia",
       capital: "Castries",
       continent: "América"
    },
    {
       id: 83,
       name: "São Cristóvão e Neves",
       capital: "Basseterre",
       continent: "América"
    },
    {
       id: 84,
       name: "São Vicente e Granadinas",
       capital: "Kingstown",
       continent: "América"
    },
    {
       id: 85,
       name: "Suriname",
       capital: "Paramaribo",
       continent: "América"
    },
    {
       id: 86,
       name: "Trindade e Tobago",
       capital: "Porto de Espanha",
       continent: "América"
    },
    {
       id: 87,
       name: "Uruguai",
       capital: "Montevideu",
       continent: "América"
    },
    {
       id: 88,
       name: "Venezuela",
       capital: "Caracas",
       continent: "América"
    },
    {
       id: 89,
       name: "Afeganistão",
       capital: "Cabul",
       continent: "Ásia"
    },
    {
       id: 90,
       name: "Arábia Saudita",
       capital: "Riade",
       continent: "Ásia"
    },
    {
       id: 91,
       name: "Arménia",
       capital: "Erevã",
       continent: "Ásia"
    },
    {
       id: 92,
       name: "Azerbaijão",
       capital: "Bacu",
       continent: "Ásia"
    },
    {
       id: 93,
       name: "Bangladexe",
       capital: "Daca",
       continent: "Ásia"
    },
    {
       id: 94,
       name: "Barém",
       capital: "Manama",
       continent: "Ásia"
    },
    {
       id: 95,
       name: "Brunei",
       capital: "Bandar Seri Begauã",
       continent: "Ásia"
    },
    {
       id: 96,
       name: "Butão",
       capital: "Timbu",
       continent: "Ásia"
    },
    {
       id: 97,
       name: "Camboja",
       capital: "Pnom Pene",
       continent: "Ásia"
    },
    {
       id: 98,
       name: "Catar",
       capital: "Doa",
       continent: "Ásia"
    },
    {
       id: 99,
       name: "Cazaquistão",
       capital: "Astana",
       continent: "Ásia"
    },
    {
       id: 100,
       name: "China",
       capital: "Pequim",
       continent: "Ásia"
    },
    {
       id: 101,
       name: "Coreia do Norte",
       capital: "Pionguiangue",
       continent: "Ásia"
    },
    {
       id: 102,
       name: "Coreia do Sul",
       capital: "Seul",
       continent: "Ásia"
    },
    {
       id: 103,
       name: "Cuaite",
       capital: "Cidade do Cuaite",
       continent: "Ásia"
    },
    {
       id: 104,
       name: "Emirados Árabes Unidos",
       capital: "Abu Dabi",
       continent: "Ásia"
    },
    {
       id: 105,
       name: "Estado da Palestina",
       capital: "Jerusalém Oriental",
       continent: "Ásia"
    },
    {
       id: 106,
       name: "Filipinas",
       capital: "Manila",
       continent: "Ásia"
    },
    {
       id: 107,
       name: "Geórgia",
       capital: "Tebilíssi",
       continent: "Ásia"
    },
    {
       id: 108,
       name: "Iémen",
       capital: "Saná",
       continent: "Ásia"
    },
    {
       id: 109,
       name: "Índia",
       capital: "Nova Déli",
       continent: "Ásia"
    },
    {
       id: 110,
       name: "Indonésia",
       capital: "Jacarta",
       continent: "Ásia"
    },
    {
       id: 111,
       name: "Irão",
       capital: "Teerão",
       continent: "Ásia"
    },
    {
       id: 112,
       name: "Iraque",
       capital: "Bagdade",
       continent: "Ásia"
    },
    {
       id: 113,
       name: "Israel",
       capital: "Jerusalém",
       continent: "Ásia"
    },
    {
       id: 114,
       name: "Japão",
       capital: "Tóquio",
       continent: "Ásia"
    },
    {
       id: 115,
       name: "Jordânia",
       capital: "Amã",
       continent: "Ásia"
    },
    {
       id: 116,
       name: "Laus",
       capital: "Vienciana",
       continent: "Ásia"
    },
    {
       id: 117,
       name: "Líbano",
       capital: "Beirute",
       continent: "Ásia"
    },
    {
       id: 118,
       name: "Malásia",
       capital: "Cuala Lumpur",
       continent: "Ásia"
    },
    {
       id: 119,
       name: "Maldivas",
       capital: "Malé",
       continent: "Ásia"
    },
    {
       id: 120,
       name: "Mianmar",
       capital: "Nepiedó",
       continent: "Ásia"
    },
    {
       id: 121,
       name: "Mongólia",
       capital: "Ulã Bator",
       continent: "Ásia"
    },
    {
       id: 122,
       name: "Nepal",
       capital: "Catmandu",
       continent: "Ásia"
    },
    {
       id: 123,
       name: "Omã",
       capital: "Mascate",
       continent: "Ásia"
    },
    {
       id: 124,
       name: "Paquistão",
       capital: "Islamabade",
       continent: "Ásia"
    },
    {
       id: 125,
       name: "Quirguistão",
       capital: "Bisqueque",
       continent: "Ásia"
    },
    {
       id: 126,
       name: "Singapura",
       capital: "Singapura",
       continent: "Ásia"
    },
    {
       id: 127,
       name: "Síria",
       capital: "Damasco",
       continent: "Ásia"
    },
    {
       id: 128,
       name: "Sri Lanca",
       capital: "Sri Jaiavardenapura-Cota",
       continent: "Ásia"
    },
    {
       id: 129,
       name: "Tailândia",
       capital: "Banguecoque",
       continent: "Ásia"
    },
    {
       id: 130,
       name: "Taiuã",
       capital: "Taipé",
       continent: "Ásia"
    },
    {
       id: 131,
       name: "Tajiquistão",
       capital: "Duchambé",
       continent: "Ásia"
    },
    {
       id: 132,
       name: "Turcomenistão",
       capital: "Asgabate",
       continent: "Ásia"
    },
    {
       id: 133,
       name: "Turquia",
       capital: "Ancara",
       continent: "Ásia"
    },
    {
       id: 134,
       name: "Usbequistão",
       capital: "Tasquente",
       continent: "Ásia"
    },
    {
       id: 135,
       name: "Vietname",
       capital: "Hanói",
       continent: "Ásia"
    },
    {
       id: 136,
       name: "Albânia",
       capital: "Tirana",
       continent: "Europa"
    },
    {
       id: 137,
       name: "Alemanha",
       capital: "Berlim",
       continent: "Europa"
    },
    {
       id: 138,
       name: "Andorra",
       capital: "Andorra-a-Velha",
       continent: "Europa"
    },
    {
       id: 139,
       name: "Áustria",
       capital: "Viena",
       continent: "Europa"
    },
    {
       id: 140,
       name: "Bélgica",
       capital: "Bruxelas",
       continent: "Europa"
    },
    {
       id: 141,
       name: "Bielorrússia",
       capital: "Minsque",
       continent: "Europa"
    },
    {
       id: 142,
       name: "Bósnia e Herzegovina",
       capital: "Saraievo",
       continent: "Europa"
    },
    {
       id: 143,
       name: "Bulgária",
       capital: "Sófia",
       continent: "Europa"
    },
    {
       id: 144,
       name: "Chipre",
       capital: "Nicósia",
       continent: "Europa"
    },
    {
       id: 145,
       name: "Cosovo",
       capital: "Pristina",
       continent: "Europa"
    },
    {
       id: 146,
       name: "Croácia",
       capital: "Zagrebe",
       continent: "Europa"
    },
    {
       id: 147,
       name: "Dinamarca",
       capital: "Copenhaga",
       continent: "Europa"
    },
    {
       id: 148,
       name: "Eslováquia",
       capital: "Bratislava",
       continent: "Europa"
    },
    {
       id: 149,
       name: "Eslovénia",
       capital: "Liubliana",
       continent: "Europa"
    },
    {
       id: 150,
       name: "Espanha",
       capital: "Madrid",
       continent: "Europa"
    },
    {
       id: 151,
       name: "Estónia",
       capital: "Talim",
       continent: "Europa"
    },
    {
       id: 152,
       name: "Finlândia",
       capital: "Helsínquia",
       continent: "Europa"
    },
    {
       id: 153,
       name: "França",
       capital: "Paris",
       continent: "Europa"
    },
    {
       id: 154,
       name: "Grécia",
       capital: "Atenas",
       continent: "Europa"
    },
    {
       id: 155,
       name: "Hungria",
       capital: "Budapeste",
       continent: "Europa"
    },
    {
       id: 156,
       name: "Irlanda",
       capital: "Dublim",
       continent: "Europa"
    },
    {
       id: 157,
       name: "Islândia",
       capital: "Reiquiavique",
       continent: "Europa"
    },
    {
       id: 158,
       name: "Itália",
       capital: "Roma",
       continent: "Europa"
    },
    {
       id: 159,
       name: "Letónia",
       capital: "Riga",
       continent: "Europa"
    },
    {
       id: 160,
       name: "Listenstaine",
       capital: "Vaduz",
       continent: "Europa"
    },
    {
       id: 161,
       name: "Lituânia",
       capital: "Vílnius",
       continent: "Europa"
    },
    {
       id: 162,
       name: "Luxemburgo",
       capital: "Luxemburgo",
       continent: "Europa"
    },
    {
       id: 163,
       name: "Macedónia",
       capital: "Escópia",
       continent: "Europa"
    },
    {
       id: 164,
       name: "Malta",
       capital: "Valeta",
       continent: "Europa"
    },
    {
       id: 165,
       name: "Moldávia",
       capital: "Quixinau",
       continent: "Europa"
    },
    {
       id: 166,
       name: "Mónaco",
       capital: "Mónaco",
       continent: "Europa"
    },
    {
       id: 167,
       name: "Montenegro",
       capital: "Podgoritsa",
       continent: "Europa"
    },
    {
       id: 168,
       name: "Noruega",
       capital: "Oslo",
       continent: "Europa"
    },
    {
       id: 169,
       name: "Países Baixos",
       capital: "Amesterdão",
       continent: "Europa"
    },
    {
       id: 170,
       name: "Polónia",
       capital: "Varsóvia",
       continent: "Europa"
    },
    {
       id: 171,
       name: "Portugal",
       capital: "Lisboa",
       continent: "Europa"
    },
    {
       id: 172,
       name: "Reino Unido",
       capital: "Londres",
       continent: "Europa"
    },
    {
       id: 173,
       name: "República Checa",
       capital: "Praga",
       continent: "Europa"
    },
    {
       id: 174,
       name: "Roménia",
       capital: "Bucareste",
       continent: "Europa"
    },
    {
       id: 175,
       name: "Rússia",
       capital: "Moscovo",
       continent: "Europa"
    },
    {
       id: 176,
       name: "São Marinho",
       capital: "São Marinho",
       continent: "Europa"
    },
    {
       id: 177,
       name: "Sérvia",
       capital: "Belgrado",
       continent: "Europa"
    },
    {
       id: 178,
       name: "Suécia",
       capital: "Estocolmo",
       continent: "Europa"
    },
    {
       id: 179,
       name: "Suíça",
       capital: "Berna",
       continent: "Europa"
    },
    {
       id: 180,
       name: "Ucrânia",
       capital: "Quieve",
       continent: "Europa"
    },
    {
       id: 181,
       name: "Vaticano",
       capital: "Vaticano",
       continent: "Europa"
    },
    {
       id: 182,
       name: "Austrália",
       capital: "Camberra",
       continent: "Oceania"
    },
    {
       id: 183,
       name: "Fiji",
       capital: "Suva",
       continent: "Oceania"
    },
    {
       id: 184,
       name: "Ilhas Marechal",
       capital: "Majuro",
       continent: "Oceania"
    },
    {
       id: 185,
       name: "Micronésia",
       capital: "Paliquir",
       continent: "Oceania"
    },
    {
       id: 186,
       name: "Nauru",
       capital: "Iarém",
       continent: "Oceania"
    },
    {
       id: 187,
       name: "Nova Zelândia",
       capital: "Wellington",
       continent: "Oceania"
    },
    {
       id: 188,
       name: "Palau",
       capital: "Ngerulmud",
       continent: "Oceania"
    },
    {
       id: 189,
       name: "Papua Nova Guiné",
       capital: "Porto Moresby",
       continent: "Oceania"
    },
    {
       id: 190,
       name: "Quiribáti",
       capital: "Taraua do Sul",
       continent: "Oceania"
    },
    {
       id: 191,
       name: "Salomão",
       capital: "Honiara",
       continent: "Oceania"
    },
    {
       id: 192,
       name: "Samoa",
       capital: "Apia",
       continent: "Oceania"
    },
    {
       id: 193,
       name: "Timor-Leste",
       capital: "Díli",
       continent: "Oceania"
    },
    {
       id: 194,
       name: "Tonga",
       capital: "Nucualofa",
       continent: "Oceania"
    },
    {
       id: 195,
       name: "Tuvalu",
       capital: "Funafuti",
       continent: "Oceania"
    },
    {
       id: 196,
       name: "Vanuatu",
       capital: "Porto Vila",
       continent: "Oceania"
    }
 ]
 
 const input = process.argv[2]
 const result = countries.filter((country)=>{
     return country.name.includes(input)

 })
 console.table(result)