...
const getPresentialMarketingPeople = (people: Colab[]) => {
    return people.filter((person) => {
        if (person.setor === Setores.MARKETING && person.presencial === true) {
            return person;
        }
    });
};
console.log(getPresentialMarketingPeople(colaborators));